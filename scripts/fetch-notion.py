...
ACTIVE_STATUSES = [
    'Planning', 'In Progress', 'Deployment', 'Business Development',
    'RFP Preparation', 'Bid Submitted', 'Bid Awarded',
]
...
def query_notion():
    ...
    while True:
        req = urllib.request.Request(
            NOTION_API, data=json.dumps(body).encode(),
...
def get_select(prop):
    if not prop or not prop.get('select'): return ''
    return prop['select'].get('name', '') if prop['select'] else ''

def get_status(prop):
    if not prop or not prop.get('status'): return ''
